...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑道具出租</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>道具出租管理</span></li>
								<li class="breadcrumb-item active"><span>编辑道具出租</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">道具出租信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>道具名称</label>
										<input name="daojumingcheng" id="daojumingcheng" name="daojumingcheng" class="form-control" placeholder="道具名称">
									</div>
									<div class="form-group col-md-6">
										<label>道具类型</label>
										<select id="daojuleixingSelect" name="daojuleixing" class="form-control">
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>图片</label>
									...
									<div class="form-group col-md-6">
										...
										<input name="xiangxidizhi" id="xiangxidizhi" name="xiangxidizhi" class="form-control" placeholder="详细地址">
											...
											<div class="form-group  col-md-12" >
												<label>道具详情</label>
												<div><div class="upload"><input id="daojuxiangqingupload" name="file" type="file">选择文件</div>
												<script id="daojuxiangqingEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var daojuxiangqingUe = UE.getEditor('daojuxiangqingEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="daojuxiangqing" id="daojuxiangqing-input">
												</div>
											</div>	
	...
	<script>
		...
		var daojuleixingOptions = [];

		function daojuleixingSelect(){
			http("option/daojuleixing/daojuleixing","GET",{},(res)=>{
				if(res.code == 0){
					daojuleixingOptions = res.data;
					var nullOption = document.createElement('option');
					var daojuleixingSelect = document.getElementById('daojuleixingSelect');
					daojuleixingSelect.appendChild(nullOption);
					for(var i=0;i<daojuleixingOptions.length;i++){
						var daojuleixingOption = document.createElement('option');
						daojuleixingOption.setAttribute('name','daojuleixingOption');
						daojuleixingOption.setAttribute('value',daojuleixingOptions[i]);
						daojuleixingOption.innerHTML = daojuleixingOptions[i];									
						if(ruleForm.daojuleixing == daojuleixingOptions[i]){
							daojuleixingOption.setAttribute('selected','selected');
						}
						daojuleixingSelect.appendChild(daojuleixingOption);
					}
				}
			});
		}	
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "daojuchuzu/info/" + id,
		...
		function upload(){
			...
			$('#daojuxiangqingupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('daojuxiangqingEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("daojuchuzu/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("daojuchuzu/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.daojuxiangqing != null && ruleForm.daojuxiangqing != 'null' && ruleForm.daojuxiangqing != ''){
				var daojuxiangqingMes = '' + ruleForm.daojuxiangqing;
				var daojuxiangqingUeditor = UE.getEditor('daojuxiangqingEditor');
				daojuxiangqingUeditor.ready(function() {
					daojuxiangqingUeditor.setContent(daojuxiangqingMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('daojuxiangqingEditor').hasContents()){
				$('#daojuxiangqing-input').attr('value',UE.getEditor('daojuxiangqingEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("daojuchuzu/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#daojumingcheng").val(ruleForm.daojumingcheng);	
			var daojuleixingOptions = document.getElementsByClassName("daojuleixingOption");
			for(var daojuleixingCount = 0; daojuleixingCount < daojuleixingOptions.length;daojuleixingCount++){
				if(daojuleixingOptions[daojuleixingCount].getAttribute('value') == ruleForm.daojuleixing){
					daojuleixingOptions[daojuleixingCount].setAttribute('selected','selected');
				}
			}
			...
			$("#daojuxiangqing").val(ruleForm.daojuxiangqing);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.daojumingcheng != null && res.data.daojumingcheng != ''){

						$("#daojumingcheng").val(res.data.daojumingcheng);
						$("#daojumingcheng").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.daojuleixing != null && res.data.daojuleixing != ''){

						var daojuleixingOptions = document.getElementsByClassName("daojuleixingOption");
						for(var daojuleixingCount = 0; daojuleixingCount < daojuleixingOptions.length;daojuleixingCount++){
							if(daojuleixingOptions[daojuleixingCount].getAttribute('value') == res.data.daojuleixing){
								daojuleixingOptions[daojuleixingCount].setAttribute('selected','selected');
							}
						}
					}
					...
					if(res.data != null && res.data != '' && res.data.daojuxiangqing != null && res.data.daojuxiangqing != ''){

						$("#daojuxiangqing").val(res.data.daojuxiangqing);
                        var daojuxiangqingMes = '' + res.data.daojuxiangqing;
                        var daojuxiangqingUeditor = UE.getEditor('daojuxiangqingEditor');
                        daojuxiangqingUeditor.ready(function() {
                            daojuxiangqingUeditor.setContent(daojuxiangqingMes);
                        });
