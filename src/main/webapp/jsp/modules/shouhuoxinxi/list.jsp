...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">收货信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>收货信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">收货信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<input type="text" id="daojumingchengSearch" class="form-control form-control-sm" placeholder="请输入道具名称" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('daojumingcheng')">道具名称<i id="daojumingchengIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('daojuleixing')">道具类型<i id="daojuleixingIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('fahuoshijian')">发货时间<i id="fahuoshijianIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			if($('#daojumingchengSearch').val() != null && $('#daojumingchengSearch').val() != ''){
				searchForm.daojumingcheng ="%" +  $('#daojumingchengSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("shouhuoxinxi/page","GET",{
				...
				daojumingcheng : searchForm.daojumingcheng,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var daojumingchengCell  = document.createElement('td');

						 daojumingchengCell.innerHTML = item.daojumingcheng;

			row.appendChild(daojumingchengCell);
			var daojuleixingCell  = document.createElement('td');

						 daojuleixingCell.innerHTML = item.daojuleixing;

			row.appendChild(daojuleixingCell);
			...
			var wuliuzhuangtaiCell  = document.createElement('td');

						  if(item.wuliuzhuangtai != null && item.wuliuzhuangtai != "" && item.wuliuzhuangtai.length >= 11){
			  	wuliuzhuangtaiCell.innerHTML = item.wuliuzhuangtai.substring(0, 10) + "..."; 
			  }else{
			  	wuliuzhuangtaiCell.innerHTML = item.wuliuzhuangtai;
			  }


			row.appendChild(wuliuzhuangtaiCell);
			var zhanghaoCell  = document.createElement('td');

						 zhanghaoCell.innerHTML = item.zhanghao;

			row.appendChild(zhanghaoCell);
			var fahuoshijianCell  = document.createElement('td');

						  fahuoshijianCell.innerHTML = item.fahuoshijian;


			row.appendChild(fahuoshijianCell);
		...
		function sh(){
			...
			httpJson("shouhuoxinxi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("shouhuoxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"shouhuoxinxi");
